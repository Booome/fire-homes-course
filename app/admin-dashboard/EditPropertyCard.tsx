...
import { Property, PropertyStatusList } from "@/lib/types";
...
function PropertyCardImpl({
  ...
  const dialogButtonClasses = "uppercase tracking-widest";
  ...
  const handleNewProperty = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      let response = await dbClient?.models.Property.create({
      ...
      response = await dbClient?.models.Property.update({
        id: propertyId,
  ...
  const handleEditProperty = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const response = await dbClient?.models.Property.update({
        id: property!.id,
        status: values.status,
        addressLine1: values.addressLine1,
        addressLine2: values.addressLine2,
  ...
  const handleDeleteProperty = async () => {
    try {
      ...
      await dbClient?.models.Property.delete({ id: property!.id });
